...
class QCMetricsController(AbstractBaseController):
    ...
    class Meta:
        ...
        arguments = [
            ...
            (['qc_root'], dict(help="Input root qc directory", default=None, nargs="?")),
    ...
    def _collect_casava_qc(self):
        qc_objects = []
        runinfo_csv = os.path.join(os.path.abspath(self.pargs.qc_root), self.pargs.flowcell, "{}.csv".format(self._fc_id()))
        ...
        fcdir = os.path.join(os.path.abspath(self.pargs.qc_root), self.pargs.flowcell)
    ...
    @controller.expose(help="Upload QC metrics to couchdb")
    def upload_qc(self):
        if not self._check_pargs(['flowcell', 'qc_root', 'url']):
            return
        path = os.path.join(self.pargs.qc_root, self.pargs.flowcell)
        if self.pargs.pre_casava:
            self._collect_pre_casava_qc()
        else:
