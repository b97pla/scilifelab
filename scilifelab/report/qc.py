...
def application_qc(project_id=None, flowcell=None, application=None,
    ...
    qc_data = p_con.get_qc_data(prj_summary, flowcell)
    ...
    if not prj_summary is None:
        ...
        qc_data = _get_sample_qc_data(prj_summary, application, s_con, flowcell)

    output_data = _qc_info_header(prj_summary, application, output_data)
...
def fastq_screen(project_id=None, flowcell=None,
                 username=None, password=None, url=None, **kw):
    output_data = {'stdout':StringIO(), 'stderr':StringIO()}
    s_con = SampleRunMetricsConnection(username=username, password=password, url=url)
