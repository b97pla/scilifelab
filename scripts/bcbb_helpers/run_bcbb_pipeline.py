...
    
    ...
    maggie_url=couch_credentials.get('maggie_url','anonymous')
    maggie_port=couch_credentials.get('maggie_port','anonymous')
    maggie_username = couch_credentials.get('maggie_username','anonymous')
    maggie_password = couch_credentials.get('maggie_password','nopassword')
...
            sample_config = bcbb_configuration_from_samplesheet(samplesheet, maggie_url, maggie_port, maggie_username, maggie_password )
             
...
    
def bcbb_configuration_from_samplesheet(csv_samplesheet, url, port, username, password):
...
    
	...
	try:
        p_con = ProjectSummaryConnection(dbname='projects', username=username, password=password, url=url, port=port)
    except:
...
        for plex in lane.get('multiplex',[]):
            try:
                ...
                application=''
            if application=="RNA-seq (total RNA)":
                plex['analysis'] = 'RNA-seq'
            elif plex.get('genome_build','') == 'hg19':
