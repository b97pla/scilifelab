...
Options:
	-e, --single_end
  	...
  	-c, --config-file = <config file>   	   
        ...
        --stranded         to include strandness table with % of strand-specifically mapped reads
...
def main(run,project_id,sample_names,single_end,config_file,Map_Stat,Read_Dist,FPKM,rRNA_table, stranded, strandness_table):
    TEMPLATE="""\
...
"""
    if strandness_table:
        TEMPLATE=TEMPLATE+"""
Percentage of strand-specificly mapped reads for the samples
^^^^^^^^^^^^^^^^^^^^^
...
def generate_report(proj_conf,single_end, stranded):
    ...
    try:
        tab = Texttable()
        tab.set_cols_dtype(['t','t'])
        tab.add_row(["Sample","%strand-specific reads"])
        try:
		f=open('strandness.json', 'rb')
        	#data=json.load(f)        
        except:
		print "can't open strandness.json\n"
	try:
		data=json.load(f)
	except:
...
if __name__ == "__main__":
    ...
    parser.add_option( "--stranded", dest="stranded",action="store_true", default=False)
    ...
    kwargs = dict(single_end = options.single_end,
		...
		stranded = options.stranded,
   	     	#strandness_table = False
		strandness_table = options.stranded  # need to have separate options for stranded in general
		#and having strandness_table in the report 
		)
