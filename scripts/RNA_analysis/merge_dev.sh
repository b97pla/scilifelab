...
Output:
    ...
    for name in ${name_list[*]};do
        ...
        if [[ -e ${flowcells[0]}/$samp_dir && -e ${flowcells[1]}/$samp_dir ]];then
            ...
            echo samtools merge $path/merged/$samp_dir/$bam_file $path/${flowcells[0]}/$samp_dir/$bam_file $path/${flowcells[1]}/$samp_dir/$bam_file
            ...
            echo "cp ${flowcells[0]}/tophat_out_$name to merged"
            ...
            echo "cp ${flowcells[1]}/tophat_out_$name to merged"
            ## copy reads and counts
    ...
    for ((i=2; i<$nr_fc; i++));do
        for name in ${name_list[*]};do       
            ...
            if [[ -e ${flowcells[i]}/$samp_dir && -e merged/$samp_dir ]];then
                ## merge read
                ##echo "merge ${flowcells[$i]}/$samp_dir/$bam_file with merged/$samp_dir/temp_${bam_file}"
                echo samtools merge $path/merged/$samp_dir/temp_${bam_file} $path/merged/$samp_dir/$bam_file $path/${flowcells[$i]}/$samp_dir/$bam_file
                ...
                ## copy reads and counts
                echo "cp ${flowcells[$i]}/$samp_dir/$bam_file to merged/"
    ...
    for i in ${rerun[*]};do
        ...
        make_MarkDup_HT_cuff.py $i $gtf_file $mail $path/merged $config_file
    done
    fi
fi
