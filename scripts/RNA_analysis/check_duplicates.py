...
def make_dupl_analysis(NAME):
	...
	duprem = open(str('tophat_out_' + NAME + '/' + NAME + '.counts'), 'r')
	withdup = open(str('tophat_out_' + NAME + '/' + NAME + '_WithDupl.counts'), 'r')
	...
	for line in withdup:
	        (key, val) = line.split()
	        if key[0]=='E':
	                withdup_dict[key] = int(val)
	
	for line in duprem:
	        (key, val) = line.split()
	        if key[0]=='E':
	                duprem_dict[key] = int(val)
	...
	Total counts				{16}
	Total clunts duplicates removed		{15}
	
	Top 3 genes, (Duplicates remooved)
	----------------------------------
	        gene			counts(duprem)		counts(with dup)		diff		fract dupl/(total NO counts)
	1.      {0}		{3}		{6}			{9}			{12}		
	2.      {1}		{4}		{7}			{10}			{13}
	3.      {2}		{5}		{8}			{11}			{14}
	==========================================
...
if len(sys.argv)!=1:
        print """
To be run from the analysis directory intermediate."""
	sys.exit()
...
for name in names:
	print name
        try:
                make_dupl_analysis(name)
        except:
                print 'could not make analysis for sample :     '+ name                
		pass	
