    ...
    for samp in info['samples']:
        if samp.strip('F')==name.strip('F'):
            ...
            try:
                ...
                size=nfo['samples'][samp.strip('F')]['library_prep'][prep]['library_validation'][key]['average_size_bp']
...
quantifyer_version      = conf['custom_algorithms'][analysis_type]['quantifyer_version']

#htseq
counts_version          = conf['custom_algorithms'][analysis_type]['counts_version']
counts_stranded         = conf['custom_algorithms'][analysis_type]['counts_stranded']

#other stuff
dup_remover_version     = conf['custom_algorithms'][analysis_type]['dup_remover_version']
picard_version          = conf['custom_algorithms'][analysis_type]['picard_version']
picard_tools            = conf['custom_algorithms'][analysis_type]['picard_tools']
BEDTools_version        = conf['custom_algorithms'][analysis_type]['BEDTools_version']
bedGraphToBigWig        = conf['custom_algorithms'][analysis_type]['bedGraphToBigWig']
...
tophat -o tophat_out_{1} {15} -p 8 {20} {8} {6} {7} {9} {10}
cd tophat_out_{1}
{13}
...
python -m HTSeq.scripts.count -s {21} -q accepted_hits_sorted_dupRemoved_prehtseq_{1}.sam {12}  > {1}.counts
samtools index accepted_hits_sorted_dupRemoved_{1}.bam
cufflinks -p 8 {20} -G {12} -o cufflinks_out_{1} accepted_hits_sorted_dupRemoved_{1}.bam

{14}

rm accepted_hits_sorted_{1}.bam
rm accepted_hits_sorted_dupRemoved_prehtseq_{1}.sam""".format(hours ,samp ,mail ,aligner_version,quantifyer_version,counts_version,innerdist,refpath,innnerdistflagg,R1,R2,dup_remover_version,annopath,sam_bam,make_fai, qscale,picard_version,picard_tools,BEDTools_version, extra_arg, aligner_libtype, counts_stranded)
        f.close()
