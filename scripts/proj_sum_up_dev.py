...
def get_proj_inf(WS_projects,project_name_swe, samp_db, proj_db, client, config):
	...
	print WS_projects.keys()	
        if len(feed.entry) != 0:
                ssheet = feed.entry[0].title.text
                info=get_20135_info(client,ssheet,WS_projects)
...
def get_WS_info(client):
        ...
        for ssheet in feed.entry:
                ...
                if ssheet_name[0:3].lower()== 'ws1':
                        info=get_20135_info(client,ssheet_name,info)
        return info


def get_20135_info(client,ssheet, info={}):
        ...
        for j, row in enumerate(content):
			...
			print row[Project_name_col]
				...
				proj=str(row[Project_name_col]).strip
                                ...
                                if not info.has_key(proj) and ''.join(prep_info_list) !='':
                                        info[proj]={sci_name:{prep:prep_info_list}}
                                elif info[proj].has_key(sci_name) and ''.join(prep_info_list) !='':
                                        info[proj][sci_name][prep] = prep_info_list
...
def strip_scilife_name(names):
        ...
        for name_init in names:
                prep = ''
                indexes = ['_index','_rpi','_agilent','_mondrian','_haloht','_halo','_sureselect','_dual','_hht','_ss','_r','_a','_m','_h']
