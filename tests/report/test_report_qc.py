...
class TestReportQCFunctions(unittest.TestCase):
    ...
    def test_application_qc(self):
        ...
        data = application_qc(application="reseq", project_name=self.examples["project"], flowcell=self.examples["flowcell"].split("_")[-1], username=self.user, password=self.pw, projectdb="projects-test", sampledb="samples-test", url=self.url)
    ...
    def test_fqscreen(self):
        ...
        data = fastq_screen(project_name=self.examples["project"], flowcell=self.examples["flowcell"].split("_")[-1], username=self.user, password=self.pw, dbname="samples-test", url=self.url)
