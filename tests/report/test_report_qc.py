...
class TestReportQCFunctions(unittest.TestCase):
    ...
    def test_application_qc(self):
        ...
        data = application_qc(application="reseq", project_id=self.examples["project"], flowcell=self.examples["flowcell"], username=self.user, password=self.pw, projectdb="projects-test", sampledb="samples-test", url=self.url)
    ...
    def test_fqscreen(self):
        ...
        data = fastq_screen(project_id=self.examples["project"], flowcell=self.examples["flowcell"], username=self.user, password=self.pw, dbname="samples-test", url=self.url)
        self.assertEqual(len(data['stdout'].getvalue().split()), 3)
