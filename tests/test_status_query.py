...
import scripts.bcbb_helpers.status_query as sq
...
class TestMetadata(unittest.TestCase):
    ...
    def test__get_flowcelldirs(self):
        ...
        self.assertListEqual([], sq._get_flowcelldirs(self.rootdir),
        ...
        self.assertListEqual(fcdirs,sorted(sq._get_flowcelldirs(self.rootdir)),
                             "Listing available flowcells did not return the 3 expected folders")
        
        # Test that filtering based on flowcell name works
        self.assertListEqual([fcdirs[-1]],sq._get_flowcelldirs(self.rootdir,os.path.basename(fcdirs[-1])),
        ...
        self.assertListEqual([fcdirs[-1]],sq._get_flowcelldirs(self.rootdir,os.path.basename(fcdirs[-1][10:])),
    ...
    def test__get_samplesheet(self):
        ...
        self.assertIsNone(sq._get_samplesheet(self.rootdir),
        ...
        for s in ss:
            utils.touch_file(s)
            self.assertEqual(s,sq._get_samplesheet(fcdir),
    ...
    def test__get_projects(self):
        ...
        os.close(fh)
        self.assertListEqual([],sq._get_projects(ssheet),
        ...
        self.assertListEqual(sorted(projects.keys()),sorted(sq._get_projects(ssheet)),
                             "The list of projects does not match the original list")
        
        # Assert that the list of projects returned is filtered as specified
        self.assertListEqual([projects.keys()[-1]],sq._get_projects(ssheet,projects.keys()[-1]),
    ...
    def test__get_project_analysis_dir(self):
        ...
        self.assertIsNone(sq._get_project_analysis_dir(self.rootdir,proj),
        ...
        os.mkdir(projdir)
        self.assertEqual(projdir,sq._get_project_analysis_dir(self.rootdir,proj),
    ...
    def test__get_project_samples(self):
        ...
        self.assertListEqual([],sq._get_project_samples(ssheet,td.generate_project()),
        ...
        for proj, sample in samples.items():
            self.assertListEqual(sorted(sample),sorted(sq._get_project_samples(ssheet,proj)),
    ...
    def test__get_sample_log(self):
        ...
        self.assertIsNone(sq._get_sample_log(self.rootdir,sample),
        ...
        self.assertEqual(os.path.join(self.rootdir,"{}-bcbb.log".format(sample)),sq._get_sample_log(self.rootdir,sample),
    ...
    def test__get_pipeline_indicator(self):
        ...
        self.assertListEqual([],sq._get_pipeline_indicator(self.rootdir),
        ...
        self.assertListEqual(sorted(ifiles),sorted(sq._get_pipeline_indicator(self.rootdir)),
        ...
        self.assertListEqual(sorted(ifiles[0:2]),sorted(sq._get_pipeline_indicator(self.rootdir,range(1,3))),
        ...
        self.assertListEqual([ifiles[-1]],sorted(sq._get_pipeline_indicator(self.rootdir,range(len(ifiles),len(ifiles)+2))),
    ...
    def test__get_most_recent(self):
        ...
        self.assertEqual((datetime.datetime.fromtimestamp(0.0),None),sq._get_most_recent([]),
        ...
        os.close(fh)
        self.assertEqual((datetime.datetime.fromtimestamp(0.0),None),sq._get_most_recent([tfile]),
        ...
        self.assertEqual((datetime.datetime.fromtimestamp(4000.0),tfiles[-1]),sq._get_most_recent(tfiles),
        ...
        self.assertEqual((datetime.datetime.fromtimestamp(5000.0),tfiles[0]),sq._get_most_recent(tfiles),
    ...
    def test__fastq_screen_finished(self):
        ...
        self.assertFalse(sq._fastq_screen_finished(self.rootdir),
        ...
        with open(sample_file,"w") as fh:
            for n in range(5):
                fh.write("{}\n".format(str(n)))
        self.assertTrue(sq._fastq_screen_finished(self.rootdir),
    ...
    def test__get_slurm_jobid(self):
        ...
        self.assertListEqual([],sq._get_slurm_jobid("jobname"),
        ...
        for jobids in [[123456789],[123456789,987654321]]:
            ...
            self.assertListEqual(jobids,sq._get_slurm_jobid("jobname"),
