...
class PmHsMetricsTest(PmTest):
    def test_1_hsmetrics(self):
        """Run hs metrics"""
        self.app = self.make_app(argv=['production', 'hs-metrics', '-f', '120829_SN0001_0001_AA001AAAXX', 'J.Doe_00_01', '--region_file', 'regionfile', '--force', '-n'], extensions=['scilifelab.pm.ext.ext_hs_metrics'])
        handler.register(ProductionController)
        self._run_app()
        hsmetrics_str = "(DRY_RUN): java -Xmx3g -jar {}/CalculateHsMetrics.jar INPUT={}/120829_SN0001_0001_AA001AAAXX/1_120829_AA001AAAXX_nophix_1-sort-dup.bam TARGET_INTERVALS=regionfile BAIT_INTERVALS=regionfile OUTPUT={}/120829_SN0001_0001_AA001AAAXX/1_120829_AA001AAAXX_nophix_1-sort-dup.hs_metrics VALIDATION_STRINGENCY=SILENT".format(os.getenv("PICARD_HOME"), self.app.config.get("production", "root"), self.app.config.get("production", "root"))
    ...
    def test_2_hsmetrics_empty(self):
        ...
        self.app = self.make_app(argv=['production', 'hs-metrics', '-f', '120829_SN0001_0001_AA001AAAXX', 'J.Doe_00_02', '--region_file', 'regionfile', '--force', '-n'], extensions=['scilifelab.pm.ext.ext_hs_metrics'])
        handler.register(ProductionController)
        self._run_app()
    ...
    def test_3_hsmetrics_drmaa(self):
        ...
        self.app = self.make_app(argv=['production', 'hs-metrics', '-f', '120829_SN0001_0001_AA001AAAXX', 'J.Doe_00_01', '--region_file', 'regionfile', '--force',  '-A', 'jobaccount', '--drmaa', '-n'], extensions=['scilifelab.pm.ext.ext_hs_metrics', 'scilifelab.pm.ext.ext_distributed'])
        handler.register(ProductionController)
        self._run_app()
        hsmetrics_str = "(DRY_RUN): java -Xmx3g -jar {}/CalculateHsMetrics.jar INPUT={}/120829_SN0001_0001_AA001AAAXX/1_120829_AA001AAAXX_nophix_1-sort-dup.bam TARGET_INTERVALS=regionfile BAIT_INTERVALS=regionfile OUTPUT={}/120829_SN0001_0001_AA001AAAXX/1_120829_AA001AAAXX_nophix_1-sort-dup.hs_metrics VALIDATION_STRINGENCY=SILENT".format(os.getenv("PICARD_HOME"), self.app.config.get("production", "root"), self.app.config.get("production", "root"))
        self.eq(hsmetrics_str, str(self.app._output_data['stderr'].getvalue().split("\n")[0]))
